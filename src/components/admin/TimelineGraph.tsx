    ...
    <div className="space-y-4">
            ...
            >
              ...
              <div
                className={`w-full rounded-t transition-all duration-300 ${
                  isToday
                    ? 'bg-burgundy-old hover:bg-burgundy-old/80'
                    : 'bg-burgundy-old/60 hover:bg-burgundy-old'
                }`}
                style={{ height: `${height}%`, minHeight: day.count > 0 ? '4px' : '0px' }}
              >
                {/* Tooltip on hover */}
                <div className="opacity-0 group-hover:opacity-100 transition-opacity absolute -translate-y-full mb-2 bg-deep-charcoal text-ivory text-xs px-2 py-1 rounded-sm whitespace-nowrap pointer-events-none z-10">
      ...
      <div className="flex items-center justify-center gap-4 text-xs text-warm-taupe">
        <div className="flex items-center gap-1">
          <div className="w-3 h-3 bg-burgundy-old/60 rounded-sm"></div>
          <span>Jour normal</span>
        </div>
        <div className="flex items-center gap-1">
          <div className="w-3 h-3 bg-burgundy-old rounded-sm"></div>
